...
def cvxCRV_rewardsconvex():
    ...
    crv_earned = convex_claim.functions.earned(public_key).call()

    # 3crv rewards are pooled stablecoins.
    three_crv_rewards = cvxCRV_3crv_reward(w3, public_key)

    # CVX token amount is calculated based on CRV earnings.
    cvx_rewards = cvx_reward_based_on_crv(w3, crv_earned)

    return {
        "CRV": Web3.fromWei(crv_earned, "ether"),
        "3Crv": Web3.fromWei(three_crv_rewards, "ether"),
        "CVX": Web3.fromWei(cvx_rewards, "ether"),
    }
...
def veCRV_rewards():
    ...
    # )
    # print(earned_amount)
    reg_provider = get_contract(w3, registry_provider, registry_provider_abi)
...
def swap(from_token, to_token):
    ...
    router_swap = get_contract(w3, uniswap_router, uniswap_router_abi)
    deadline = time.time() + 1000000
    router_swap.functions.swapExactETHForTokens(
        1, [WETH, BAT], public_key, int(deadline)
    ).call()
    # swap_contract = get_contract(w3, bat_weth, bat_weth_abi)
    # swap_contract
